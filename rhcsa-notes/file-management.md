...
> - **Relative path**: This is relative to the current directory, shown with the `pwd` command. It's possible to get from this directory up to the file that's needed.
...
The name of the file, however, is not stored in the inode. All file names are instead stored within a directory. In fact, inodes are unaware of the name assigned to them, it only knows how many names are associated with that single inode. These are what constitute a hard link. Every file always has one hard link to begin with, which is the name of the file.

The creation of a file means giving it a name, which then becomes that file's hard link. Many hard links can be linked to a file, which is useful if the contents of said file need to be accessible at different locations around the system. 

When a change is made to any of the hard links, it will then reflect on all other hard links, as the links point to the same block of data. There are some restrictions, however:

- Hard links must be on the same device (lvm, partitions)
- It's not possible to create hard links to directories. 
- Once the last name of a file is removed, it will no longer be possible access the contents of that file. 

There is no difference between one hard link and another. Removing one hard link does not remove the other hard links attached to that file.

### Symbolic Links

Commonly also called a soft link, doesn't directly link to an inode, but instead to the name of a file. For this reason, soft links are a lot more flexible, although there are some drawbacks to keep in mind.

- The main advantage of a soft link is that it can link to files on a different device, as well as to a different directory. 
- The biggest drawback is that upon the event that the original file is removed, the link becomes invalid and will no longer function.

### How to Create Links

Creating links is done with the `ln` command, and uses similar options and parameters to the `mv` and `cp` commands. You start by providding the source name and then the name of the destination. 

In order to create a symbolic link, the `-s` option is added to the `ln` command. Here is a table showing some common examples. 

| Description | Command |
| --- | --- |
| Creates a new link to the directory /tmp from the current directory | ln /tmp . |
| Creates a soft link to the /tmp directory from the current directory | ls -s /tmp . |

Use the `ls` command to see whether a file is a link. If it is, it will show an `l` at the beginning of the line when using `ls -l`. If the link is a symbolic link, `ls -l` will show the name of the directory that the link connects to. If it's a hard link, the same command will instead show a counter, which is the number shown before the owner of the file. 

### Deleting Links

It's important to be cautious when deleting links. The following example exemplifies why this is so:

1. Create a directory called dir within the home directory using the command: `mkdir ~/dir`.
2. Copy all of the files that start with a,b,c,d,e from /etc to this directory. This can be done with the command: `cp /etc/[a-e]* ~/dir`.
3. Use `ls -l ~/dir` to make sure sure of the contents of the new directory. 
4. Switch to the home directory with `cd ~`.
5. Use `ln -s dir slink`. 
6. Remove the link with `rm slink`, which will then delete the symbolic link.
7. Use `ls -l` again and it will show that the soft link has been successfully removed. 
8. Use `ln -s dir slink` again to recreate the link.
9. Use `rm -rf slink/`. 
10. Type `ls`, which will show that the link still does exist. 
11. Using `ls dir/` will show that the directory is empty. 
