...
When a destination is specified, a file is typically used. If the filename begins with an `-`, the messages will not be immediately written to the file, but instead will be buffered, making the process more efficient. It's also possible to make use of device files, such as `/dev/console`, which will make messages write to the console. This is not as common as it once is as admins that are logging into a server will not be able to view that messages that are being writtent to the server's console.

The syslog facilities were first defined in the 1980s, and no new facilities have been added since to ensure that there is backward compatibility, meaning that there are some facilities that no longer serve much of a purpose. In order to determine the kinds of messages that are meant to be logged, it's possible to assign different severities in `rsyslog.conf` lines. These are the severities in ascending order:

| Priority | Description |
| --- | --- |
| debug | Debug messages that give detailed information about a service |
| info | Messages about normal service operation |
| notice | Messages about items that might cause an issue at a later stage |
| warning (warn) | There is no error but it's not working as well as it could |
| error (err) | A noncritical error has occurred |
| crit | A critical error has occurred |
| alert | Message used when a service is about to be discontinued |
| emerg (panic) | Generated when the a service has been discontinued |

When a priority is used, all messages with that priority and higher are logged according to the specification of that specific rule. It's also possible to log messages with different priorities sent to different files by using the `=` sign. Here is an example where all cron messages that have the debug priority will be written to a specific file with the name `/var/log/cron.debug`. Notice that the `-` in front of the line specifies to buffer writes so that infomation is logged in a more efficient way. 

```sh
cron.=debug `/var/log/cron.debug
```

To read more about the different facilities and priorities of rsyslog, use the `man 5 rsyslog.conf` command.

## Log File Rotation

It's a good idea to rotate log files to ensure that log files don't fill up a system. When a certain threshold is met, old log files are closed and new ones are opened. When a log file is rotated, the old file is copied to a file that has the rotation date in it. If `/var/log/messages` is rotated on the 7th of July 2026, the rotated filename will be `/var/log/messages-2026-07-07`. By default, four old log files are kept on the system. Files that are older than this will be automatically removed from the system. The default settings for log file rotation are kept at `/etc/logrotate.conf`. Try the following command to see what the settings are:

```bash
cat /etc/logrotate.conf
```

It's also possible to learn more about log rotation by using the `man logrotate` command. If there are specific files that need more fine-grained settings, it's possible to create a configuration file at `/etc/lograte.d`. Any settings added to this file will overwrite the default settings `/etc/lograte.conf`.  
